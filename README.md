...
1)make .env file in the root folder for all needed env variables. List of variables you can find in constants.js
2)Docker service must be installed on the server and started.
3)Create docker imgage.
...
--network=host - needed if any(or all) databases are on the localhost, if they are on other server you can use ```-p 9090:9090``` to publish server on any port. 
